...
def _verify_code(db: Session, code: str):
    if not code or len(code) != 6:
        raise HTTPException(status_code=401, detail="invalid code")
    # naive check: any row exists and active
    row = db.query(ClientAuth).filter(ClientAuth.is_active == True).first()
    if not row:
        raise HTTPException(status_code=403, detail="no active client codes")
    return True
...
def clients_by_code(db: Session = Depends(get_db), access_code: str = Query(None), code: str = Query(None)):
    _verify_code(db, access_code or code or "")
    ...
    row = db.query(ClientAuth).first()
    return {"description": row.description if row else "", "is_active": bool(row.is_active) if row else False,
